...
def get_calendar_service(request_flow = False):
...
async def connect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('is_connecting'):
        await update.message.reply_text("Connection already in progress...")
        return
    
    await update.message.reply_text("⌛ A login tab is opening on your web browser. Please authorize the bot there.")
    
    try:
        context.user_data['is_connecting'] = True
        flow = InstalledAppFlow.from_client_secrets_file('credentials.json', SCOPES)
        creds = flow.run_local_server(port=8080)
        with open('token.json', 'w') as token:
            token.write(creds.to_json())

        context.user_data['is_connecting'] = False
